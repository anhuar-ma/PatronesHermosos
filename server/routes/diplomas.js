  ...
  try {
    ...
    if (userRole === 0) {
      ...
      for (const sede of sedesResult.rows) {
        const sedeDir = path.join(tempDir, `sede_${sede.id_sede}_${sede.nombre}`);
        ...
        for (const coord of coordResult.rows) {
          const diplomaPath = path.join(sedeDir, `coordinadora_${coord.id_coordinadora}.pdf`);
        ...
        for (const coordAsoc of coordAsocResult.rows) {
          const diplomaPath = path.join(sedeDir, `coordinadora_asociada_${coordAsoc.id_coordinadora_asociada}.pdf`);
          await generateDiploma(
      ...
      const tempDir = path.join(process.cwd(), `temp_diplomas_sede_${sedeId}`);
      ...
      for (const participante of participantesResult.rows) {
        const diplomaPath = path.join(participantesDir, `participante_${participante.id_participante}.pdf`);
        await generateDiploma(
      ...
      for (const mentora of mentorasResult.rows) {
        const diplomaPath = path.join(mentorasDir, `mentora_${mentora.id_mentora}.pdf`);
        await generateDiploma(
      ...
      for (const colaborador of colaboradoresResult.rows) {
        ...
        const diplomaPath = path.join(outputDir, `${colaborador.rol.toLowerCase()}_${colaborador.id_colaborador}.pdf`);
        await generateDiploma(
